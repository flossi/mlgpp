...
def FFP_clim(zm=None, z0=None, umean=None, h=None, ol=None, sigmav=None, ustar=None,
    ...
    if n==0:
        ...
        if fig:
            fig_out,ax = plot_footprint(x_2d=x_2d, y_2d=y_2d, fs=fclim_2d,
                                        show_heatmap=show_heatmap,clevs=frs,clim=True)
...
def plot_footprint(x_2d, y_2d, fs, clevs=None, show_heatmap=True, normalize=None, 
                   colormap=None, line_width=0.5, iso_labels=None, clim=False):
    ...
    if clevs is not None:
        ...
        for f, c in zip(fs, cs):
            ...
            if iso_labels is not None:
                pers = [str(int(l*100))+'%' for l in iso_labels][::-1]
                fmt = {}
                for l,s in zip(cp.levels, pers):
                    fmt[l] = s
                plt.clabel(cp, cp.levels[:], inline=1, fmt=fmt, fontsize=10)
    ...
    if show_heatmap:
        ...
        if clim == False:
            for f in fs:
                im = plt.pcolormesh(x_2d, y_2d, f, cmap=colormap, norm=norm)
...
def plot_footprint_after(ffp_dict, show_heatmap=True, normalize=None, colormap=None,
                         line_width=0.5, iso_labels=False, clim=False):
    '''Plot footprint function and contours if request. If multiple contour
    leves are to be drawn, these must be supplied in ascending order to the
    flux footprint estimation function!'''

    # If input is a list of footprints, don't show footprint but only contours,
    # with different colors
    x_2d = ffp_dict['x_2d']
    y_2d = ffp_dict['y_2d']
    fs = ffp_dict['fclim_2d']
    clevs = ffp_dict['fr']
    # largest (i.e. last) contour level is used for xlim & ylim
    xoffs = [round(xoff, 1) for xoff in ffp_dict['xr'][-1]]
    yoffs = [round(yoff, 1) for yoff in ffp_dict['yr'][-1]]
    ...
    if clevs is not None:
        ...
        for f, c in zip(fs, cs):
            ...
            # Isopleth Labels
            if iso_labels == True:
                pers = [str(int(l*100))+'%' for l in ffp_dict['rs']][::-1]
                fmt = {}
                for l,s in zip(cp.levels, pers):
                    fmt[l] = s
                plt.clabel(cp, cp.levels[:], inline=1, fmt=fmt, fontsize=10)
    ...
    if show_heatmap:
        ...
        cbar = fig.colorbar(im, shrink=1.0, format='%.3e')
        #cbar.set_label('Flux contribution', color = 'k')
    ax.set_xlim([min(xoffs)-50, max(xoffs)+50])
    ax.set_ylim([min(yoffs)-50, max(yoffs)+50])
    plt.show()
